...
struct PixelBuffer {
	/**
	  * Formats that you'll have to deal with if MainWindow decides to give you a buffer with such a format.
	  * Color converion is not handled by MainWindow, because it would have to support n² format conversions
	  * rather than the n conversions a typical source will have to support. I may add support for providing
	  * a list of preferred formats, so that in the case that multiple frame buffer formats are equally supported
	  * you will get the one that has highest preference.
	  *
