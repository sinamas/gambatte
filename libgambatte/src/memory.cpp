...
void Memory::init() {
	...
	memset(memory + 0xC000 + 0x00, 0xFF, 0x08);
	memset(memory + 0xC000 + 0x08, 0x00, 0x08);
	memcpy(memory + 0xC000 + 0x10, memory + 0xC000, 0x800 - 0x10);
	
	memset(memory + 0xC800 + 0x00, 0x00, 0x08);
	memset(memory + 0xC800 + 0x08, 0xFF, 0x08);
	memcpy(memory + 0xC800 + 0x10, memory + 0xC800, 0x600);
	
	memory[0xCE00 + 0x02] = 0xFF;
	memory[0xCE00 + 0x0A] = 0x00;
	memcpy(memory + 0xCE00 + 0x10, memory + 0xCE00, 0x200 - 0x10);
	...
	cgb_bgp_data[0x00] = 0xFF;
	cgb_bgp_data[0x01] = 0x7F;
	memcpy(cgb_bgp_data + 0x02, cgb_bgp_data, 0x40 - 0x02);
